...
-- Segment
local Segment = Object:subclass("mcnet.transport.Segment")
...
function Segment:serializeFlags()
	local aFlags = {}
...
local SendQueue = EventEmitter:subclass("mcnet.transport.SendQueue")
...
function Connection:rawSend(flags, data)
	...
	self.transport:rawSend(self.destAddress, segment)
end
...
function Transport:initialize(address)
	...
	self.listenHandlers = {}
	self.connections = {}
	self.network = Network:new(self.address)
end
...
function Transport:rawSend(destAddress, segment)
	self.network:send(destAddress, Segment:serialize(segment))
end
function Transport:getListenHandler(sourcePort)
	return self.listenHandlers[sourcePort]
end
function Transport:isListening(sourcePort)
	return self:getListenHandler(sourcePort) ~= nil
end
function Transport:listen(sourcePort, handler)
